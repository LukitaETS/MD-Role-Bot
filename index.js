...
const testDiscordToken = process.env.TEST_DISCORD_TOKEN === 'true';
const enableGuildMembersIntent = process.env.ENABLE_GUILD_MEMBERS_INTENT !== 'false';
...
const pruebaToken = {
  estado: testDiscordToken ? 'pendiente' : 'desactivado',
  detalle: testDiscordToken
    ? 'Todavia no se probo el token contra la API REST de Discord.'
    : 'Prueba REST desactivada para evitar rate limits de Discord.',
  codigoHttp: null,
...
console.log(
  enableGuildMembersIntent
    ? 'Intent GuildMembers activado. Recuerda activar SERVER MEMBERS INTENT en Discord Developer Portal.'
    : 'Intent GuildMembers desactivado por ENABLE_GUILD_MEMBERS_INTENT=false.'
);

// Intents usados por el proyecto: servidores, mensajes de servidor y miembros si esta habilitado.
...
function iniciarServidorSalud() {
  ...
  const server = http.createServer((request, response) => {
    const respuesta = {
      ...
      guildMembersIntentActivado: enableGuildMembersIntent,
...
async function probarTokenDiscord() {
  ...
  pruebaToken.retryAfterSegundos = null;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      pruebaToken.retryAfterSegundos = retryAfter ? Number(retryAfter) : null;
      pruebaToken.estado = 'rechazado';
      if (response.status === 401) {
        pruebaToken.detalle = 'Discord rechazo el token. Regenera DISCORD_TOKEN en Developer Portal.';
      } else if (response.status === 429) {
        pruebaToken.detalle =
          'Discord aplico rate limit. Espera unos minutos y evita redeploys repetidos.';
      } else {
        pruebaToken.detalle = `Discord respondio con HTTP ${response.status}.`;
      }
      console.error(`Error: la prueba REST del token fallo con HTTP ${response.status}.`);
      if (pruebaToken.retryAfterSegundos) {
        console.error(`Discord pidio esperar ${pruebaToken.retryAfterSegundos} segundo(s) antes de reintentar.`);
      }
      return;
    }
...
if (!token) {
  ...
  if (testDiscordToken) {
    ...
    console.log('Prueba REST del token desactivada por TEST_DISCORD_TOKEN=false.');
  }
